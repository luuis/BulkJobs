package extra;

public class tets {
    public static void main(String[] args) {
        /* JSONObject json = GeoAPI.search("Sierra Hermosa, Tecámac, México");
        System.out.println(json); */
        
        System.out.println("CL\tCreating email");
        Email em = new Email();
        System.out.println("CL\tBuilding email");
        em.setRemitente("luuis.e@outlook.com");
        em.setDestinatario("imluuis.e@gmail.com");
        em.setAsunto("Prueba 3");
        em.setCuerpo("<h1>This is actual message <i>embedded</i> in HTML tags</h1>");
        
        System.out.println("CL\tCalling EM");
        em.enviarCorreo();
        System.out.println("CL\tFinish sending");
        
        System.exit(0);
    }
}
